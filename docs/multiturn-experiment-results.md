...
checkpoint-325 进入完整 dev500×3 同协议复核，但尚未冻结为 GRPO 基座。dev200 上 checkpoint-406 的总 strict 为 61.17%，而既有完整 dev500 结果为 52.13%，说明只凭该子集冻结模型仍有风险。在 checkpoint-325 完成 dev500 复核前，不创建 `selected-for-grpo` 记录、不启动 GRPO，也不使用最终 200-task 评测集。
