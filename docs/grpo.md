...
规范数据仍通过 prepare_multiturn_grpo_dataset.py 从冻结的 GRPO task split 与匹配 openings 生成 parquet，并使用所有开发/正式评测任务作为 exclude-tasks。生成物的 manifest、task 计数、task-disjoint 审计和 SHA-256 必须在训练前冻结。
