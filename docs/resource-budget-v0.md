...
- 状态：静态预估；5 条 persona 多轮轨迹 Pilot 后用实测更新
- 模型：Qwen3.5-2B
...
| Teacher 与 Shopper 输入输出 token、提问次数和成功率 | 计算正式多轮轨迹采集成本 |
...
1. 至少 5 条 ShopSimulator persona Teacher 多轮购物轨迹跑通；
2. 对这些轨迹完成 tokenizer 长度统计；
3. 在目标 4×4090 服务器完成一次 SFT forward/backward smoke；
4. 完成 5-step GRPO smoke。
