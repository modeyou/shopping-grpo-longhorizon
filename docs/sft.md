...
checkpoint-200 当前领先，但还不是正式冻结的 GRPO 基座。final-2epoch 在 gap+ask 上提高 2.8 个百分点，却在 complete+ask 上下降 35.2 个百分点，并产生大量 error。当前正在固定 dev200 子集上筛选 checkpoint-250/300/350，判断第二个 epoch 内的行为拐点。

dev200 sweep manifest SHA-256：

```text
5754aaaf1a4b67c47751f4e35782866a4794d49910bbcf9651eff2d5080b2d1a
```

在 sweep 完成并对领先候选执行完整 dev500×3 前，不创建 `selected-for-grpo` 记录，也不启动 GRPO。最终 200-task 评测集继续封存。
