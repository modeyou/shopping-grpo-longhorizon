...
`instruction_simple`，可作为原论文 schema 的本地参照。但当前 HTTP 服务初始化环境时没有开启
persona 模式，项目客户端 reset 也没有多轮 Shopper 协议；当前训练/评测 prompt 反而明确禁止追问。

正式项目因此采用以下继承边界：

- 继承：冻结商品目录、购物环境动作、商品事实和可验证终局；
- 参考但不作为正式梯度数据：原作者 SFT/GRPO 轨迹和内嵌画像；
- 自行生成：正式画像、当前请求、私有目标、澄清回答、教师轨迹、数据划分和审计清单；
- 自行实现：用户交互 orchestrator 与训练/评测所需的统一状态传递。
