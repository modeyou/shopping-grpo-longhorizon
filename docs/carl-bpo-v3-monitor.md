...
> 当前状态（2026-09-03）：run `sconfuhu` 已在 `global_step_200` 保存后人工停止并转入评测，
> 因此本页实时命令用于续训或下一次运行。当前 step200 的结论见
> [CARL-BPO 第19节](carl-bpo.md#19-当前-v3-进度导出合同与-dev-500-结论)。

正式训练使用 `nohup ... >"$CARL_LOG" 2>&1`，控制台的 stdout 和 stderr 都保存在完整日志里。
建议另开一个 SSH 窗口同时监控两条通道。
