#!/usr/bin/env python3
'''Apply, verify, or restore the veRL 0.8 fused-PPO gradient backport.'''
...
BACKUP_SUFFIX = '.shopping-grpo-fused-grad.orig'
...
def resolve_target():
    installed = importlib.metadata.version('verl')
    if installed != EXPECTED_VERL_VERSION:
        raise RuntimeError(f'GRPO requires verl=={EXPECTED_VERL_VERSION}, got {installed}')
    import verl
...
def verify(target):
    ...
    if target.read_text(encoding='utf-8').count(PATCH_MARKER) != 1:
        raise RuntimeError('GRPO fused-PPO gradient marker must occur exactly once')
...
def apply(target):
    ...
    if PATCH_MARKER in source:
        verify(target)
        print(f'veRL GRPO fused-PPO gradient patch already applied: {target}')
        return
    ...
    temporary = target.with_name(target.name + '.shopping-grpo-fused-grad.tmp')
    ...
    try:
        ...
        raise
    print(f'applied veRL GRPO fused-PPO gradient patch: {target}')
...
def main():
    ...
    try:
        ...
        if args.restore:
            ...
            verify(target)
            print(f'verified veRL GRPO fused-PPO gradient patch: {target}')
        else:
            apply(target)
    except (OSError, RuntimeError, ValueError, py_compile.PyCompileError) as exc:
        raise SystemExit(f'veRL GRPO fused-PPO gradient patch error: {exc}') from exc
