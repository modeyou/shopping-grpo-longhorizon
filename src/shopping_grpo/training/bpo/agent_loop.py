...
class ShoppingBPOAgentLoop(ShoppingToolAgentLoop):
    ...
    async def _new_agent_data(self, kwargs):
        ...
        if selected_names and self.tools:
            selected = {name: self.tools[name] for name in selected_names if name in self.tools}
    ...
    async def _run_clone(self, candidate, sampling_params, task_id, sibling_index):
        ...
        session = ClonedBranchSession(
            payload["source_env"], candidate.snapshot_id,
            deepcopy(payload["runtime_state"]), shopper,
        )
        await session.start()
        try:
            ...
            self._attach_bpo_metadata(
                output, candidate, action_starts, sibling_index,
                payload["group_id"],
            )
            return output
        finally:
            ...
            try:
                while current != AgentState.TERMINATED:
                    if current == AgentState.GENERATING:
                        ...
                        current = await self._handle_generating_state(data, sampling_params)
                        try:
                            pieces = self.tokenizer.convert_ids_to_tokens(data.response_ids)
                            token_offset = first_decision_token(pieces)
                            entropy = await self._probe_entropy(
                                prompt_before + data.response_ids[:token_offset], sampling_params
                            )
                        except ValueError:
                            await asyncio.to_thread(source_env.drop_snapshot, snapshot_id)
                        except Exception:
                            await asyncio.to_thread(source_env.drop_snapshot, snapshot_id)
                            raise
                        else:
                            ...
                            selected = select_branch_candidate(
                                [value for value in (candidate, proposed) if value is not None]
                            )
                ...
                if candidate is None:
                    raise RuntimeError("BPO backbone has no valid semantic decision boundary")
